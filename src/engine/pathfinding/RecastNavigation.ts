...
// 1. walkableClimb (0.8) - allows ramp traversal but blocks cliff jumps
...
//
// RAMP MATH:
// - Elevation 0-255 maps to height 0-10.2 (factor 0.04)
// - A typical ramp: 100 elevation change over 8 cells = 4.0 height over 8 units
// - Per-cell height change: 0.5 units. With cs=0.5, navmesh sees ~0.25 per sample
// - walkableClimb=0.8 allows this while still blocking cliffs (3.2+ unit jumps)
const NAVMESH_CONFIG: Partial<TileCacheGeneratorConfig> = {
  ...
  ch: 0.2,
  // Agent parameters
  // STRICT: Max 50° slopes - ramps are typically 30-40°, cliffs are 90°
  walkableSlopeAngle: 50,
  walkableHeight: 2,
  // Allow ramp traversal: 0.8 units handles typical ramps (0.4-0.6 per cell)
  // while still blocking cliff jumps (elevation differences are 3.2+ units)
  walkableClimb: 0.8,
...
// Uses same settings as tiled config for consistent cliff handling
const SOLO_NAVMESH_CONFIG: Partial<SoloNavMeshGeneratorConfig> = {
  cs: 0.5,
  ch: 0.2,
  // STRICT: Max 50° slopes - matches tiled config
  walkableSlopeAngle: 50,
  walkableHeight: 2,
  // Allow ramp traversal - matches tiled config
  walkableClimb: 0.8,
  walkableRadius: 0.6,
  maxSimplificationError: 0.5,
};
