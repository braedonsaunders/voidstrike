...
export class LineOfSight {
  ...
  /**
   * Optimized shadowcasting for high-performance LOS calculation
   * Uses recursive shadowcasting in 8 octants (similar to roguelike FOV)
   *
   * This is more efficient than ray-per-cell for large sight ranges.
   */
  public getVisibleCellsShadowcast(
    casterX: number,
    casterY: number,
    sightRange: number
  ): Set<number> {
    ...
    // Caster's cell is always visible
    if (
      casterCellX >= 0 &&
      casterCellX < this.config.gridWidth &&
      casterCellY >= 0 &&
      casterCellY < this.config.gridHeight
    ) {
      visibleCells.add(casterCellY * this.config.gridWidth + casterCellX);
    }
  ...
  private castLight(
    ...
    if (startSlope < endSlope || row > range) {
      return;
    }
    ...
    for (let distance = row; distance <= range && !blocked; distance++) {
      const dy = -distance;

      for (let dx = -distance; dx <= 0; dx++) {
        // Map octant-relative coordinates to actual coordinates
        const [mapDx, mapDy] = this.transformOctant(dx, dy, octant);
        const targetCellX = casterCellX + mapDx;
        const targetCellY = casterCellY + mapDy;
        ...
        const distSq = mapDx * mapDx + mapDy * mapDy;
        if (distSq > range * range) {
          continue;
        }

        // Check terrain height blocking
        const targetWorldX = (targetCellX + 0.5) * this.config.cellSize;
        const targetWorldY = (targetCellY + 0.5) * this.config.cellSize;
        const targetHeight = this.getHeight!(targetWorldX, targetWorldY);

        // Cell is visible if terrain height doesn't block
        const eyeHeightOffset = 0.5;
        const sightLineHeight =
          casterHeight +
          eyeHeightOffset -
          (Math.sqrt(distSq) * this.config.cellSize * eyeHeightOffset) / range;

        const isBlocking = targetHeight > sightLineHeight + this.config.losBlockingThreshold;
        ...
        if (blocked) {
          ...
          if (isBlocking && distance < range) {
            // This cell blocks - recurse
            blocked = true;
            this.castLight(
              visibleCells,
              casterCellX,
   ...
   */
  private transformOctant(dx: number, dy: number, octant: number): [number, number] {
    switch (octant) {
      case 0:
        return [dx, dy];
      case 1:
